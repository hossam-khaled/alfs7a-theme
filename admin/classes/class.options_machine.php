...
class Options_Machine {
	 ...
	 * @since 1.0.0
	 */
	public static $khafagy_sections_builder;
	public static $khafagy_categories_manager;
	public static $khafagy_quick_wizard;

	function __construct($options) {

		// Options_Machine::$khafagy_sections_builder = $GLOBALS['khafagy_sections_builder'];
		// Options_Machine::$khafagy_categories_manager = $GLOBALS['khafagy_categories_manager'];
		// Options_Machine::$khafagy_quick_wizard = $GLOBALS['khafagy_quick_wizard'];
	...
	public static function optionsframework_machine($options) {
		...
		foreach ($options as $value) {
			...
			//Start Heading
			 if ( $value['type'] != "heading" && $value['type'] != "box" && $value['type'] != "sections_builder" )
			 {
				...
				$output .= '<div id="section-'.$value['id'].'" class="'.$fold.'single-section section-'.$value['type'].' '. $class .'">'."\n";
			...
			if (!isset($smof_data[$value['id']]) && $value['type'] != "heading" && $value['type'] != "box" && $value['type'] != "sections_builder") {
				$the_default_value = isset($value['std']) ? $value['std'] : '';
				$smof_data[$value['id']] = $the_default_value;
				of_save_options( array( $value['id'] => $the_default_value ) );
			}
			...
			switch ( $value['type'] ) {
				...
				//text input
				case 'box':
					if( $box_counter > 0 ) $output .= '</div></div><!-- end box -->';
					$output .= '<!-- start box --><div class="box '.$value['class'].'"><div class="box-title">'.$value['name'].'</div><div class="inner">';
					$box_counter++;
				break;
				...
				case 'sections_builder':
					...
					if(isset($value['options'])){
						...
						$ta_value = stripslashes($smof_data[$value['id']]);
						$value['value'] = $smof_data[$value['id']];
						ob_start();
							Options_Machine::$khafagy_sections_builder->display_control_panel_page( $value );
						$output .= ob_get_clean();
				break;
				...
				case 'quick_wizard':
					...
					if(isset($value['options'])){
						...
						$ta_value = stripslashes($smof_data[$value['id']]);
						//$output .= '<input class="quick_wizard" name="' . $value['id'] . '" type="hidden" value="' . $smof_data[$value['id']] . '" />';
						ob_start();
							Options_Machine::$khafagy_quick_wizard->quick_wizard_page();
						$output .= ob_get_clean();
				break;

				case 'categories_manager':
					$cols = '8';
					$ta_value = '';
					...
					if(isset($value['options'])){
						...
						$ta_value = stripslashes($smof_data[$value['id']]);
						//$output .= '<input class="quick_wizard" name="' . $value['id'] . '" type="hidden" value="' . $smof_data[$value['id']] . '" />';
						$output .= Options_Machine::$khafagy_categories_manager->categories_manager();
				break;
				...
				case 'typography':
					...
					if(isset($typography_stored['face'])) {
						...
						$faces = array('arial'=>'Arial',
										...
										// 'palatino'=>'Palatino',
										'Droid Arabic naskh'=>'naskh',
										'Droid Arabic Kufi'=>'Kufi',
				...
				case 'heading':
					...
					if(isset($value['icon'])){
						...
						$icon = ' style="background-image: url( '. ADMIN_IMAGES . 'icon-' . $value['class'] .'.png);"';
					}
					$header_class = isset( $value['class'] ) ? $value['class'] : str_replace(' ','',strtolower($value['name']));
					$jquery_click_hook = isset( $value['class'] ) ? $value['class'] : str_replace(' ', '', strtolower($value['name']) );
					$jquery_click_hook = "of-option-" . $jquery_click_hook;
					$menu .= '<li class="'. $header_class .'"><a title="'.  $value['name'] .'" href="#'.  $jquery_click_hook  .'"'. $icon .'>'.  $value['name'] .'</a></li>';
					$output .= '<div class="'. $header_class .'-container group" id="'. $jquery_click_hook  .'"><h2>'.$value['name'].'</h2>'."\n";
				break;
				...
				case 'backup':
					...
					//$output .= var_dump(get_option(BACKUPS));
					$output .= '</div>';
				...
				case 'sliderui':
					...
					//html output
					$output .= '<input type="hidden" name="'.$value['id'].'" id="'.$value['id'].'" value="'. $s_val .'" class="mini" '. $s_edit .' />';
				...
				case 'switch':
					...
					$output .= '<p class="switch-options'.$cb_enabled.'">';
						$output .= '<label class="'.$fold.'cb-enable" data-id="'.$value['id'].'"><span>'. $on .'</span></label>';
						$output .= '<label class="'.$fold.'cb-disable" data-id="'.$value['id'].'"><span>'. $off .'</span></label>';
	...
	public static function optionsframework_media_uploader_function($id,$std,$mod){
		...
		if ( function_exists( 'wp_enqueue_media' ) ) {
			$uploader .= '<span class="button media_upload_button" id="'.$id.'">رفع</span>';

			if(!empty($upload)) {$hide = '';} else { $hide = 'hide';}
			$uploader .= '<div class="button remove-image '. $hide.'" id="reset_'. $id .'" title="' . $id . '">حذف</div>';
		}
		else
		{
